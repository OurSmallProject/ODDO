...
router.get('/all', (req, res) => {
  Event.find()
    ...
    .catch((err) => res.status(404).json({ error: 'Error in get api/events/all. ' + err }));
});

// GET /api/events/events
// or /api/events/events?sport=${sport}
// fetch all events or by type of event that are not flag
router.get('/events', (req, res) => {
  Event.find(req.query.sport ? { typeofsport: req.query.sport, flag: false } : { flag: false })
    ...
    .catch((err) => res.status(404).json({ error: 'Error in get api/events/events. ' + err }));
});
...
router.get('/:id', (req, res) => {
  Event.findById(req.params.id)
    ...
    .catch((err) => res.status(404).json({ error: 'Error in get api/events/:id. ' + err }));
});
...
router.post('/', (req, res) => {
  ...
  const foundUser = User.findById('60a6ab2fd6a82d8e2dde0694');
  console.log(foundUser);

  const eventFields = {};
  eventFields.user = foundUser.id;
...
router.put('/:id/flag', (req, res) => {
  Event.findById('60a6d45fee56d3a70a090775')
    .then((event) => {
      if (!event) {
        return res.status(404).json({ error: 'This event is not found' });
      }
